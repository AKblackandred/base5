...
PrincipleAxesModel::PrincipleAxesModel() {
	...
	paShader = setupShaders(string("assets\\shaders\\basic_shader.vs.txt"), string(""), string("assets\\shaders\\basic_shader.fs.txt"));
...
void PrincipleAxesModel::render(const glm::mat4& T) {
	...
	// Setup transform for my position
	mat4 modelMatrix = mat4(1.0f);
	modelMatrix = translate(modelMatrix, pos);

	mat4 mvp = T * modelMatrix;
...
vec3 PrincipleAxesModel::getPosition()
{
	return pos;
}

void PrincipleAxesModel::setPosition(vec3 newPosition)
{
	pos = newPosition;
}